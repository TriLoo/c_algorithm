...
inline Vector2d cam2px(const Vector3d cam)
{
    return Vector2d((cam(0, 0) * fx) / cam(2, 0) + cx, (cam(1, 0) * fy) / cam(2, 0) + cy);
}
...
{
    return pt(0, 0) >= boarder && pt(0, 0) <= width - boarder && pt(1, 0) >= boarder && pt(1, 0) <= height - boarder;
}
...
)
{
    ...
    while (!fin.eof())
    {
        ...
        poses.push_back(SE3(Quaterniond(data[6], data[3], data[4], data[5]), Vector3d(data[0], data[1], data[2])));
...
)
{
    ...
    f_ref.normalize();                  // 归一化坐标，下同
    Vector3d f_curr = px2cam(pt_curr);
    f_curr.normalize();

    Vector3d t = T_R_C.translation();
    Vector3d f2 = T_R_C.rotation_matrix() * f_curr;    // Rx2
    ...
    double alpha = acos(f_ref.dot(t) / t_norm);    // f_ref.norm = 1
    double beta = acos(-a.dot(t) / (a_norm * t_norm));
    double beta_prime = beta + atan(1/fx);         // delta_beta, see p327
    double gamma = M_PI - alpha - beta_prime;
    double p_prime = t_norm * sin(beta_prime) / sin(gamma);
    double d_cov = p_prime - depth_estimation;
    double d_conv2 = d_cov * d_cov;

    //  高斯融合
    double mu = depth.ptr<double>(int(pt_ref(1, 0)))[int(pt_ref(0, 0))];
    double sigma2 = depth_cov.ptr<double>( int(pt_ref(1, 0)) )[ int(pt_ref(0, 0)) ];

    double mu_fuse = (d_conv2 * mu + sigma2 * depth_estimation) / (sigma2 + d_conv2);
    double sigma_fuse2 = (sigma2 * d_conv2) / (sigma2 + d_conv2);

    depth.ptr<double>(int(pt_ref(1, 0)))[int(pt_ref(1, 0))] = mu_fuse;
    depth_cov.ptr<double>(int(pt_ref(1, 0)))[int(pt_ref(1, 0))] = sigma_fuse2;
...
int main(int argc, char ** argv) {
    ...
    if ( argc != 2 )
    {
        cout<<"Usage: DenseMapSingle path_to_test_dataset"<<endl;
        return -1;
    }
